...
Figure~\ref{fig:dashboard-page} is the first thing the user sees after logging in. The dashboard page was developed to receive backend dashboard data and show stock health, reorder planning, stock cycling, delivery pipeline, purchase planning, popular products, finance, cashflow, and order coverage. It is also connected to other pages, so a low-stock product can lead the user to the inventory or product page, and an order issue can lead to the purchase or sales page.
...
Figure~\ref{fig:product-management-form} shows the product form where the user manages item details. The products page was developed as the main product catalog, including product creation, editing, disabling, deleting where allowed, SKUs, previous SKUs, names, category links, unit conversions, reorder levels, product pictures, supplier links, average cost information, and product transaction history. Products with transaction history can be disabled instead of removed, which protects old purchase, sale, and quotation records.
...
Figure~\ref{fig:sales-workflow} shows how a customer sale is prepared and tracked. The sales page was developed for customer-side transactions, including sale headers, customer information, customer purchase-order reference, sale items, documents, VAT totals, discounts, and delivery-related statuses. Sale item statuses include pending, packed, shipped, delivered, cancelled, and returned. The backend validates stock before a sale can commit stock, and when stock is committed, allocations are created from received purchase layers.
...
Figure~\ref{fig:quotation-workflow} shows a quotation before it turns into a real purchase or sale. The quotation page was developed for quotation line items, supplier cost options, validity settings, shipping date, payment terms, VAT mode, total values, and conversion to purchase or sales flow. In the database, quotation items are stored in the \texttt{QuotationItem} table and supplier options are stored in \texttt{QuotationItemSupplier}, while the API still returns an \texttt{items} array for frontend compatibility.
...
Figure~\ref{fig:billing-note-workflow} shows how eligible sales are grouped into a billing note for customer invoicing. The billing notes page was developed for customer receivable tracking. It loads backend-controlled eligible sales, groups them into billing notes, tracks whether each line has been received, and shows summary values, status filters, expected payment dates, actual payment dates, bank references, and linked credit note information.
...
Figure~\ref{fig:payment-batch-workflow} shows the payment batch page for supplier payables. The payment batches page was developed to load eligible purchases from the backend instead of allowing users to manually select any purchase. Each payment batch stores supplier information, planned and actual payment dates, bank reference, total amount, and paid line status, keeping supplier payment tracking in one place.
