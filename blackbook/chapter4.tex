...
\end{figure}
Figure~\ref{fig:inventory-page} is the daily stock-checking workspace. It shows how many units are on hand, which products need reordering, which suppliers offer the best price, and how quickly each item is selling — all calculated by the backend so the numbers stay reliable.
...
\end{figure}
Figure~\ref{fig:quotation-workflow} shows a quotation before it turns into a real purchase or sale. The user can compare supplier pricing, set validity dates, and review line totals — then convert the quotation into one or more purchase orders or a sales record.
