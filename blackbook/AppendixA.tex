...
Figure~\ref{fig:appendix-sidebar-navigation} shows the sidebar used after sign-in. It groups the operational, transaction, record, administration, and Settings pages so that users can move between related tasks.
...
Table~\ref{tab:appendix-master-data-checklist} lists the minimum information that should be prepared in each master-data module before transactions are entered.
...
Figure~\ref{fig:appendix-product-form} shows where product units and conversion quantities are entered. The base unit should represent the smallest unit used for stock calculation. For example, if a product is bought by box but sold by piece, the base unit may be piece and the purchase unit may be box. A conversion such as one box equals ten pieces allows purchase quantities to be converted into stock quantities correctly.
...
Figure~\ref{fig:appendix-quotation-entry} shows the quotation entry screen and the stock-sufficiency information used before conversion.
...
Figure~\ref{fig:appendix-purchase-receiving} shows the purchase detail and the receiving status that controls when purchased units enter available stock.
...
Figure~\ref{fig:appendix-sales-delivery} shows the delivery-status controls used to keep the recorded sale state consistent with the actual order state.
...
Table~\ref{tab:appendix-finance-follow-up} explains the business side and main user action for each finance follow-up module.
...
Figure~\ref{fig:appendix-dashboard-review} shows the dashboard overview used to review these business indicators together.
...
Table~\ref{tab:appendix-quick-reference} maps common user goals to the page and main action used to complete each goal.
