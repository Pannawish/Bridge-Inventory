...
The Inventory Management System was developed to address these practical workflow problems for an SME trading environment. The implemented system uses a React frontend, a Django REST Framework backend, and a MySQL relational database. Its design focuses on master data management, purchasing, sales, quotations, billing notes, payment batches, credit notes, dashboard summaries, stock validation, and AI-assisted text queries. The backend is responsible for authoritative validation and stock calculations, while the frontend provides a compact web interface for daily operational work.

The project also includes an AI-assisted inventory chat feature. Instead of requiring users to manually navigate every page, the assistant allows users to ask text questions about stock, transactions, partner summaries, receivables, payables, and business exceptions. The codebase supports local backend-derived answers and can use an OpenAI model when the required API configuration is supplied.
...
\begin{enumerate}
	...
	\item \textbf{AI-assisted operation:} The system includes a text-based AI inventory assistant endpoint. It answers inventory and transaction questions using backend operational data and can use an OpenAI model when an API key is configured.
\end{enumerate}
...
\begin{itemize}
	\item Full production deployment performance testing and high-volume concurrent-user testing.
	\item Voice-based or image-based chatbot interaction.
	\item Dedicated audit-log tables for every document status change.
	\item Confirmed implementation of Economic Order Quantity, Z-score, or a complete P-system calculation module.
	% TODO: If these inventory formulas are later implemented, update this scope statement and Chapter 4 results.
\end{itemize}
...
The system is also expected to improve operational accessibility by providing dashboard summaries and an AI-assisted text interface. Users should be able to retrieve important business information such as product stock, transaction status, receivables, payables, partner summaries, and stock exceptions without manually searching every module. Overall, the completed system is expected to reduce manual checking, improve stock visibility, and support better day-to-day decision making for an SME trading business.

% TODO: If EOQ, Z-score, P-system, or additional inventory formulas are implemented later, add them to this expected results section with matching evidence in Chapter 4.
