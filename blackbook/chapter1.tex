...
\begin{enumerate}
	...
	\item \textbf{Authentication and access flow:} The backend includes JWT login, refresh, and authenticated user profile endpoints. The frontend supports login, token refresh, logout, and guest mode. Backend-wide authentication enforcement is controlled by the \texttt{INVENTORY\_REQUIRE\_AUTH} setting. Role-based permission levels and user administration screens are outside the current project scope.
