...
Testing was focused on backend behavior because the backend owns the most important business rules. The test suite checks pagination, filters, product pictures, operational data commands, stock validation, FIFO allocation, manual allocation, purchase item status changes, quotation normalization, reference numbers, lookup endpoints, eligibility endpoints, credit notes, purchase payable synchronization, seed data, and assistant answer alignment.
...
The selected black-box aligned backend test suite contains 30 automated cases covering product validation, quotation behavior, purchase workflow, sale stock validation, finance eligibility, credit notes, payment batches, and payable synchronization. The latest recorded run passed all 30 cases. Additional focused validation was also run for the reorder point calculation, AI Chat, and AI Report. The focused validation run executed 5 tests and all 5 passed.

\begin{table}[!htbp]
	\centering
	\BlackbookTableSetup
	\caption{Final Validation Test Evidence}
	\label{tab:final-validation-test-evidence}
	\begin{tabularx}{\textwidth}{|B{3.2cm}|D{1.9cm}|Z|}
		\hline
		\textbf{Validation Item} & \textbf{Result} & \textbf{Evidence} \\ \hline
		Selected backend workflow tests & 30/30 passed & The latest recorded black-box aligned backend run passed all selected cases with no failures. \\ \hline
		Reorder point calculation & Passed & The fixed mock data returned available stock 60, average daily demand 4, safety stock 28, reorder level 48, recommended restock 18, days until stockout 15, and stock value 300. \\ \hline
		AI Chat configured-key test & Passed & With an API key setting configured, AI Chat still returned a deterministic backend summary based on the created customer and sale records. \\ \hline
		AI Report configured-key test & Passed & The report context contained the created supplier, purchase reference, and THB 450.00 amount, and the final HTML included a print button. \\ \hline
		AI Report chart and safety tests & Passed & Model-generated chart HTML was replaced with backend chart rows, and unsafe HTML such as scripts, iframes, event handlers, and JavaScript links was removed. \\ \hline
	\end{tabularx}
\end{table}
