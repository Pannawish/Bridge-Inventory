...
Security support is implemented with Simple JWT. The backend exposes login, token refresh, and authenticated profile endpoints. The frontend stores and refreshes tokens, then attaches the access token to API requests. In local development, the backend can allow unauthenticated access depending on \texttt{INVENTORY\_REQUIRE\_AUTH}; when that setting is enabled, API access requires authentication.
